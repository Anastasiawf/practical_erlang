...
-record(state, {max_rooms = 5,rooms = maps:new()}).
...
Reply;
{'DOWN', MRef, process, Server, Reason} -> {error, Reason}
after 5000 ->
erlang:demonitor(MRef, [flush]),
no_reply
end.
...
receive  
{Msg, From, Ref} -> {Reply, State2} = ?MODULE:handle_call(Msg, State),
...
create_room(Server, RoomName) -> call(Server, {create_room, RoomName}).
remove_room(Server, RoomId) -> call(Server, {remove_room, RoomId}).
get_rooms(Server) -> call(Server, {get_rooms}).
add_user(Server, RoomId, UserName) -> call(Server, {add_user, RoomId, UserName}).
remove_user(Server, RoomId, UserName) -> call(Server, {remove_user, RoomId, UserName}).
get_users_list(Server, RoomId) -> call(Server, {get_users_list, RoomId}).
send_message(Server, RoomId, UserName, Message) -> call(Server, {send_message, RoomId, UserName, Message}).
get_messages_history(Server, RoomId) -> call(Server, {messages_history, RoomId}).
